...
extern MGPIO_t MGPIO_ARR[USED_PINS_PER_PORT*USED_PORTS];
MGPIO_enuErrorStatus_t GPIO_enumInit(void){
    MGPIO_enuErrorStatus_t Local_enuErrorStatus = MGPIO_enuOK;
    for(u8 i = 0; i < USED_PINS_PER_PORT*USED_PORTS; i++){
...
MGPIO_enuErrorStatus_t GPIO_enuTogglePinValue  (MGPIO_enuPort_t Copy_u8Port, MGPIO_enuPin_t Copy_u8Pin)
{
    ...
    if(Copy_u8Port > 5){
        ...
        GPIO_enuGetPinValue(Copy_u8Port, Copy_u8Pin, &Local_u8Value);
        if(Local_u8Value == MGPIO_enuLow){
            GPIO_enuSetPinValue(Copy_u8Port, Copy_u8Pin, MGPIO_enuHigh);
        }
        else{
            GPIO_enuSetPinValue(Copy_u8Port, Copy_u8Pin, MGPIO_enuLow);
        }
    }
    return Local_enuErrorStatus;
}